...
import { Plus, Pencil, Trash2, X, Check, Home } from "lucide-react";
import { store, newId } from "../data/store";
import type { Property } from "../data/types";
import { useTheme } from "./theme";
import ImageInput from "./ImageInput";

const EMPTY: Omit<Property, "id"> = { img: "", title: "", location: "", price: "", type: "", area: "", link: "" };

function Modal({
  title, data, onSave, onClose,
}: {
  title: string; data: Omit<Property, "id">;
  onSave: (d: Omit<Property, "id">) => void; onClose: () => void;
}) {
  const T = useTheme();
    ...
    >
      <div style={{ width: 520, maxWidth: "calc(100vw - 32px)", maxHeight: "90vh", overflowY: "auto", background: T.surface, borderRadius: 18, border: `1px solid ${T.border}`, padding: "32px 32px 28px", boxShadow: T.mode === "light" ? "0 16px 48px oklch(0.4 0.01 250 / 0.15)" : "none" }}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave(form); }} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <ImageInput label="Foto do imóvel" value={form.img} onChange={(v) => setForm((f) => ({ ...f, img: v }))} />

          <div className="grid grid-cols-2 gap-3">
            {([
              { key: "title" as const, label: "Título", required: true },
              { key: "type" as const, label: "Tipo (ex: Moradia)", required: true },
              { key: "price" as const, label: "Preço (ex: € 320.000)", required: true },
              { key: "area" as const, label: "Área (ex: 120m²)", required: true },
              ...
              <div key={key}>
                ...
                <input value={form[key]} onChange={set(key)} required={required} style={inputStyle}
          ...
          <div>
            <label style={{ display: "block", fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", color: T.faint, marginBottom: 6 }}>Localização</label>
            <input value={form.location} onChange={set("location")} required style={inputStyle} placeholder="ex: Foz do Douro, Porto"
              onFocus={(e) => (e.currentTarget.style.borderColor = T.accent)}
              onBlur={(e) => (e.currentTarget.style.borderColor = T.border)} />
          </div>

          <div>
            <label style={{ display: "block", fontSize: 10, letterSpacing: "0.24em", textTransform: "uppercase", color: T.faint, marginBottom: 6 }}>Link do imóvel (opcional)</label>
...
export default function Properties() {
  const T = useTheme();
  const [items, setItems] = useState<Property[]>(store.getProperties);
  const [modal, setModal] = useState<{ mode: "add" } | { mode: "edit"; item: Property } | null>(null);

  const persist = (next: Property[]) => { setItems(next); store.setProperties(next); };
  ...
  const handleDelete = (id: string) => {
    if (confirm("Eliminar este imóvel?")) persist(items.filter((p) => p.id !== id));
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32 }}>
        <div>
          <h1 style={{ fontFamily: '"Instrument Serif", serif', fontSize: 36, color: T.text, marginBottom: 4, letterSpacing: "-0.02em" }}>Imóveis</h1>
          <p style={{ color: T.faint, fontSize: 13 }}>{items.length} imóve{items.length !== 1 ? "is" : "l"}</p>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {items.map((p) => (
          <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 16, padding: "16px 24px", background: T.surface, borderRadius: 14, border: `1px solid ${T.border}`, boxShadow: T.shadow }}>
            {p.img ? (
              <img src={p.img} alt={p.title} style={{ width: 72, height: 56, borderRadius: 8, objectFit: "cover", flexShrink: 0, border: `1px solid ${T.border}` }} />
            ) : (
              <div style={{ width: 72, height: 56, borderRadius: 8, flexShrink: 0, background: T.raised, border: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center", color: T.faint }}>
                <Home size={18} />
              </div>
            )}
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 11, color: T.faint, letterSpacing: "0.06em" }}>{p.type} · {p.area}</span>
              </div>
              <p style={{ fontSize: 13, color: T.muted, lineHeight: 1.5, margin: 0 }}>{p.location} — {p.price}</p>
            </div>
      ...
      {modal && (
        <Modal
          title={modal.mode === "add" ? "Novo imóvel" : "Editar imóvel"}
          data={modal.mode === "edit" ? { img: modal.item.img, title: modal.item.title, location: modal.item.location, price: modal.item.price, type: modal.item.type, area: modal.item.area, link: modal.item.link ?? "" } : EMPTY}
